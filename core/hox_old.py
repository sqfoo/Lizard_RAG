...
class HOX:
    def __init__(self):
        ...
        print('Setting up the Critic Agent ...')
        self.critic = Agent(
            primary_LLM=GEMINI_LITE,
            backup_LLM=HUGGINGFACE_LITE,
            tools=[
    ...
    def _core_(self, state: AgentState):
        ...
        if not llm_output:
            ...
            try:
                ...
                result = json_repair.loads(clean_content)
    ...
    def router_condition(self, state: AgentState):
        # success condition
        if state.get("final_decision") == "TRUE":
            print("\n✅ Task completed\n")
            return END
